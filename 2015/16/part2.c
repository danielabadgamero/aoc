...
int match(const char* item, int num, const char* line)
{
  ...
  int actual = strtol(line + index, NULL, 10);
  if (strcmp(item, "cats") == 0 || strcmp(item, "trees") == 0)
    return num < actual;
  else if (strcmp(item, "pomeranians") == 0 || strcmp(item, "goldfish") == 0)
    return num > actual;
  else return num == actual;
}
